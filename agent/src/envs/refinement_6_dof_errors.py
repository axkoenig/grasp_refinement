...
class ActionSpace(Space):
    ...
    def __init__(self):
        ...
        self.add_variable(2, "wrist_rel_xy", 0, -0.01, 0.01)
        self.add_variable(1, "wrist_rel_z", 0, -0.01, 0.02)
...
class GazeboEnv(gym.Env):
    def __init__(self, exec_secs, max_ep_len, joint_lim, obj_shift_tol):
        ...
        self.num_contacts = 0
        self.epsilon = 0
        self.error_reflex_x = 0.02
        self.error_reflex_y = 0.01
    ...
    def step(self, action):
        ...
        self.hand_cmd.preshape = 0
        self.hand_pub.publish(self.hand_cmd)
        self.gazebo_interface.cmd_wrist_pos_incr([action[3], action[4], action[5]])
    ...
    def reset(self):
        ...
        x_offset = np.random.uniform(- self.error_reflex_x, self.error_reflex_x)
        y_offset = np.random.uniform(- self.error_reflex_y, self.error_reflex_y)
        rospy.loginfo(f"Random offset for init wrist pose is [x: {x_offset}, y: {y_offset}].")
        mat_offset = tf.transformations.translation_matrix([x_offset, y_offset, 0])
        mat_init = np.dot(self.wrist_init_pose, mat_offset)

        self.gazebo_interface.reset_world(mat_init, self.obj_init_pose)
